...
async function test() {
  console.log('尝试 GET 方式获取消息（带参数）...');
  const result = await apiCall('GET', `/v3/app/worksheets/${CONFIG.messageWorksheet}/rows?limit=10&offset=0&include=all`, null);
  console.log('GET 结果:', JSON.stringify(result, null, 2).substring(0, 3000));
}
